...
let indent_file args = function
  | Args.InChannel ic ->
      let config, syntaxes, _dlink = IndentConfig.local_default () in
      ...
      let out, perm, need_move =
        if args.Args.inplace then
          ...
          let rec get_true_file path =
            ...
            | { st_kind = S_REG ; st_perm ; _ } -> Some tmp_file, st_perm, Some path
            | { st_kind = S_LNK ; _ } -> get_true_file @@ readlink path
            | { st_kind = _ ; _ } -> failwith "invalid file type"
