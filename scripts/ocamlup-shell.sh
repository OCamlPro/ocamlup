...
# install ocamlup. It just does platform detection, downloads the installer
# and runs it.
...
if [ "$KSH_VERSION" = 'Version JM 93t+ 2010-03-05' ]; then
    ...
    echo 'ocamlup does not work with this ksh93 version; please try bash!' >&2
    exit 1
fi
...
main() {
    ...
    local _url="${OCAMLUP_UPDATE_ROOT}/dist/${_arch}/ocamlup-init${_ext}"
    echo $_url

    local _dir
    _dir="$(ensure mktemp -d)"
    local _file="${_dir}/ocamlup-init${_ext}"
    ...
    if [ ! -x "$_file" ]; then
        ...
        printf '%s\n' "Please copy the file to a location where you can execute binaries and run ./ocamlup-init${_ext}." 1>&2
        exit 1
    fi
...
get_ciphersuites_for_curl() {
    if [ -n "${OCAMLUP_TLS_CIPHERSUITES-}" ]; then
        # user specified custom cipher suites, assume they know what they're doing
        RETVAL="$OCAMLUP_TLS_CIPHERSUITES"
        return
    fi
